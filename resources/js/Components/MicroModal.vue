    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
      <div class="modal__container w-2/3" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
        <header class="modal__header">
          ...
          <button type="button" class="modal__close" @click="toggleStatus" aria-label="Close modal" data-micromodal-close></button>
        </header>
        ...
        <footer class="modal__footer float-right">
          <button type="button" @click="toggleStatus" class="modal__btn m-auto" data-micromodal-close aria-label="Close this dialog window">閉じる</button>
        </footer>
      </div>
    </div>
  </div>
  <input name="customer" v-model="search" placeholder="タナカ" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
  <button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="flex mx-auto my-4 text-white bg-blue-300 border-0 py-2 px-8 focus:outline-none hover:bg-blue-400 rounded text-lg">検索する</button>
